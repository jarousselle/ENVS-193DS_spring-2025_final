...
For each category of bird, a 1 represents that the nest box is occupied by that type of bird and the 0 represents that it is not occupied by that type of bird or is empty. The response variables include, Swift Parrot, Common Starling, and Tree Martin.
...
```{r}
model0 <- lm(sp ~ 1, data = nest_boxes)
model1 <- lm(sp ~ edge.distance + season, 
             data = nest_boxes)
model2 <- lm(sp ~ season,
             data = nest_boxes)
model3 <- lm(sp ~ edge.distance,
             data = nest_boxes)
```
...
The best model that predicts Swift Parrot presence in nest boxes as determined by Akaike's Information Criterion (AIC) includes season and distance to the forest edge (F(2, 224) = 8.5, p < 0.001, $\alpha$ = 0.05)

### h. Visualize the model predictions

```{r}
ggpredict(model1,
          terms = c("edge.distance",
                    "season")) |> 
